	...
	var methods = {
		init : function(options) {
			var defaults = {  
				...
				fade_speed: 600, 		// Effect ducation for the 'fade' effect only
			...
			return this.each(function(){
				...
				if ($data.length) {
					...
					var $ch_wrapper = $('<div>').addClass('ch_element').addClass(settings.wrapper_class).css({ 'width':w, 'height':h, 'position':'relative', 'overflow':'hidden','margin': 'auto' }).insertAfter($this);
